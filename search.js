          ...
          handleToken: function(tag, token, fixedStartSet){
            ...
            switch(tag?tag.toLowerCase():undefined){
              ...
              case "prop":
                ...
                if(token.indexOf("=^") > 0){
                  let [p, v] = token.split("=^")
                  return (fixedStartSet?fixedStartSet:this.getAllIds()).filter((id) => (global.EntityStorage.props.getProps(id)[p] || "").toLowerCase().startsWith(v.toLowerCase()))

                } else if(token.endsWith("^")){
                  let [p, v] = token.slice(0, -1).split("=")
                  return (fixedStartSet?fixedStartSet:this.getAllIds()).filter((id) => (global.EntityStorage.props.getProps(id)[p] || "").toLowerCase().endsWith(v.toLowerCase()))
                  ...
                  if(global.EntityStorage.indices.propcontains)
                    ...
                    return (fixedStartSet?fixedStartSet:this.getAllIds()).filter((id) => (""+(global.EntityStorage.props.getProps(id)[p] || "")).toLowerCase().indexOf(v.toLowerCase())>=0)
