...
async function doSearch(query){
    ...
    for(let e of entities){
        ...
        row.append(`<td>${Object.keys(e.props).map(p => `<a href="?query=prop:${p}=${e.props[p]}">${p} = ${e.props[p]}</a>`).join("<br/>")}</td>`)
        row.append(`<td>${e.tags.map(t => `<a href="?query=tag:${t}">${t}</a>`).join(", ")}</td>`)
